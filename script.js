(() => {
  ...
  const state = {
    ...
    builder:{type:"",use:"",details:{personalizacion:[]},quantity:"",customQuantity:"",size:"",environment:[],measure:"",deadline:"",deadlineDate:"",file:"",files:[],filePreview:"",filePreviews:[],idea:""}
  };

  function persistRequest(){writeStorage(STORAGE_REQUEST,state.items.map(({referencePreview,referencePreviews,...item})=>({...item,image:String(item.image||"").startsWith("blob:")?builderStockImage(item.answers?.tipo):item.image})));}
  ...
  function checkout(){
    ...
    const fileCount=state.items.reduce((total,item)=>total+(item.files?.length||(item.file?1:0)),0);const hasFiles=fileCount>0;
    return `<section class="simple-page checkout-page"><div class="page-head" data-entry><button class="round" data-route="request" aria-label="Volver a mi solicitud"><i class="ph ph-arrow-left" aria-hidden="true"></i></button><h1>Enviar consulta</h1><span></span></div>
      ...
      <aside class="checkout-summary" data-entry><span class="section-label">RESUMEN</span><h2>${state.items.length} ${state.items.length===1?"idea":"ideas"} para cotizar</h2>${state.items.map(item=>`<p><span>${item.name}<small>${item.quantity} · ${itemAnswerRows(item)[0]?.[1]||item.size}</small></span><i class="ph ${item.icon}"></i></p>`).join("")}${hasFiles?`<div class="attachment-note"><i class="ph ph-paperclip" aria-hidden="true"></i><span><b>Último paso · ${fileCount} ${fileCount===1?"archivo":"archivos"}</b>WhatsApp abrirá el mensaje completo. Adjuntá allí las referencias indicadas; el navegador no puede enviarlas automáticamente.</span></div>`:""}<button class="whatsapp-button" id="send-whatsapp"><i class="ph ph-whatsapp-logo"></i> Abrir consulta en WhatsApp</button></aside></div>
    </section>`;
  }

  function success(){
    const detail=state.messageWasCompacted?"WhatsApp se abrió con un resumen abreviado para asegurar compatibilidad.<br>El mensaje te avisa qué detalles confirmar en el chat.":"WhatsApp se abrió con toda la información ordenada.<br>Solo falta enviarla para que podamos responderte.";
  ...
  function builderV2(){
    ...
    const types=[["Para mi marca o negocio","Llaveros, cartelería, exhibidores, porta QR...","ph-storefront"],["Para un evento","Souvenirs, decoración, identificadores...","ph-confetti"],["Una pieza funcional","Soportes, repuestos, organizadores...","ph-wrench"],["Algo personal","Regalos, figuras, decoración...","ph-gift"],["No estoy seguro/a","Te ayudamos a definirlo.","ph-question"]];
    ...
    return `<section class="builder-page builder-v2"><div class="builder-head" data-entry><button class="round" data-route="home" aria-label="Volver al inicio"><i class="ph ph-arrow-left" aria-hidden="true"></i></button><div><small>CREAR DESDE CERO</small><h1>Contanos tu idea.<br><span>Nosotros vemos cómo hacerla realidad.</span></h1><p>Respondé unas preguntas rápidas y recibimos tu idea directamente por WhatsApp.</p></div><span></span></div>
      <div class="builder-layout"><div class="builder-stage" data-entry><div class="builder-grid"></div><img id="builder-image" src="${visual}" alt="${state.builder.filePreview?"Vista previa de tu referencia":"Referencia conceptual del proyecto"}"><span class="builder-badge"><i class="ph ph-sparkle"></i> ${state.builder.filePreview?"Tu referencia":"Tu idea en proceso"}</span><div class="builder-live"><div><span><small>PROYECTO</small><strong id="builder-title">${state.builder.type||"Sin definir"}</strong></span><span><small>CANTIDAD</small><strong id="builder-quantity-live">${quantity}</strong></span><span><small>PLAZO</small><strong>${state.builder.deadline||"Sin definir"}</strong></span></div><p id="builder-brief-text">${state.builder.idea?escapeText(state.builder.idea):"Tu descripción va a aparecer acá mientras armás el proyecto."}</p><div class="builder-specs"><span><b>Uso</b><i id="builder-environment">${displayValue(state.builder.environment)||"Sin definir"}</i></span><span><b>Tamaño</b><i id="builder-measure-live">${state.builder.measure||"A definir"}</i></span><span><b>Personalización</b><i>${displayValue(personalization)||"A definir"}</i></span></div><div class="brief-progress"><span><i style="width:${progress}%"></i></span><b id="builder-progress">Brief ${progress}% completo</b></div></div></div>
      <div class="builder-controls" data-entry><div class="form-alert" id="builder-alert" role="alert" hidden></div>
        <section><small id="builder-type-label">01 · ¿QUÉ QUERÉS CREAR? *</small><div class="builder-options builder-options-five" role="group" aria-labelledby="builder-type-label">${types.map(([value,help,icon])=>`<button class="builder-option ${state.builder.type===value?"active":""}" aria-pressed="${state.builder.type===value}" data-builder="type" data-value="${value}"><span><i class="ph ${icon}"></i></span><b>${value}<small>${help}</small></b></button>`).join("")}</div></section>
        <section><small>02 · CONTANOS TU IDEA *</small><label class="builder-idea"><textarea id="builder-idea" aria-required="true" maxlength="600" rows="5" placeholder="Ej: Quiero hacer 50 llaveros con el logo de mi negocio, de unos 5 cm. Tengo el logo pero no tengo el diseño 3D.">${escapeText(state.builder.idea)}</textarea><span>No necesitás saber de diseño 3D. Contanos la idea como puedas y nosotros te ayudamos.</span></label></section>
        <section><small id="builder-personalization-label">03 · ¿QUERÉS PERSONALIZARLO?</small><div class="config-options multi-options" role="group" aria-labelledby="builder-personalization-label" data-builder-multi="personalizacion">${["Logo","Texto / nombre","Colores","No","No estoy seguro/a"].map(value=>`<button class="${personalization.includes(value)?"active":""}" aria-pressed="${personalization.includes(value)}" data-value="${value}">${value}</button>`).join("")}</div></section>
        <section><small id="builder-environment-label">04 · ¿DÓNDE O CÓMO SE VA A USAR? *</small><div class="config-options multi-options" role="group" aria-labelledby="builder-environment-label" data-builder-segment="environment">${["Interior","Exterior","En contacto con agua / humedad","Va a soportar peso o movimiento","No estoy seguro/a"].map(value=>{const active=Array.isArray(state.builder.environment)?state.builder.environment.includes(value):state.builder.environment===value;return `<button class="${active?"active":""}" aria-pressed="${active}" data-value="${value}">${value}</button>`;}).join("")}</div><label class="builder-measure"><span>Tamaño aproximado <em>(opcional)</em></span><input id="builder-measure" maxlength="120" data-builder-input="measure" value="${escapeText(state.builder.measure)}" placeholder="Ej: 10 × 15 cm"></label></section>
        <section><small>05 · ¿CUÁNTAS UNIDADES NECESITÁS? *</small><div class="segmented builder-quantity" role="group" aria-label="Cantidad" data-builder-segment="quantity">${["1","10","25","50","100","250+","Otra"].map(value=>`<button class="${state.builder.quantity===value?"active":""}" aria-pressed="${state.builder.quantity===value}" data-value="${value}">${value==="Otra"?"Otra cantidad":value}</button>`).join("")}</div>${state.builder.quantity==="Otra"?`<label class="conditional-input"><span>Cantidad exacta</span><input id="builder-custom-quantity" inputmode="numeric" aria-required="true" maxlength="5" value="${escapeText(state.builder.customQuantity)}" placeholder="Ej.: 75"></label>`:""}</section>
        <section><small>06 · ¿PARA CUÁNDO LO NECESITÁS? *</small><div class="config-options" role="group" aria-label="Plazo" data-builder-segment="deadline">${["Sin apuro","Tengo una fecha","Lo antes posible"].map(value=>`<button class="${state.builder.deadline===value?"active":""}" aria-pressed="${state.builder.deadline===value}" data-value="${value}">${value}</button>`).join("")}</div>${state.builder.deadline==="Tengo una fecha"?`<label class="conditional-input"><span>📅 Seleccioná la fecha</span><input id="builder-deadline-date" type="date" min="${todayValue()}" aria-required="true" value="${escapeText(state.builder.deadlineDate)}"></label>`:""}</section>
        <section><small>07 · MOSTRANOS TU IDEA</small><div class="builder-upload-row"><label class="builder-upload"><input id="builder-file" type="file" multiple accept=".png,.jpg,.jpeg,.svg,.pdf,.stl,.obj,.3mf,.step,.stp,.iges,.igs"><i class="ph ph-upload-simple" aria-hidden="true"></i><span><b>Subí cualquier referencia que tengas</b><small>${state.builder.files.length?`${state.builder.files.length} archivo${state.builder.files.length===1?"":"s"} seleccionado${state.builder.files.length===1?"":"s"}`:"Puede ser una foto, dibujo, logo, captura, plano o archivo 3D. (opcional)"}</small></span></label>${state.builder.files.length?`<button class="clear-references builder-clear" data-clear-builder-references aria-label="Quitar todas las referencias"><i class="ph ph-x"></i></button>`:""}</div></section>
        <button class="primary builder-add" id="builder-add">${state.editingUid?"Actualizar mi idea":"Enviar mi idea por WhatsApp"} <i class="ph ph-arrow-right" aria-hidden="true"></i></button>
      </div></div>
    </section>`;
  }
  ...
  function addBuilderV2(){
    state.builder.idea=view.querySelector("#builder-idea")?.value.trim()||"";
    ...
    if(!validQuantity(state.builder.quantity,state.builder.customQuantity))missing.push("Cantidad mayor que cero");
  ...
  function bindEvents(){
    ...
    view.querySelector("#send-whatsapp")?.addEventListener("click",()=>{state.customerName=view.querySelector("#customer-name").value.trim();state.customerContext=view.querySelector("#customer-context").value.trim();state.customerMessage=view.querySelector("#customer-message").value.trim();if(!state.customerName){notify("Decinos tu nombre para continuar");view.querySelector("#customer-name").focus();return;}let message=buildMessage();state.messageWasCompacted=encodeURIComponent(message).length>7000;if(state.messageWasCompacted)message=buildMessage(true);const url=`https://wa.me/${PHONE}?text=${encodeURIComponent(message)}`;state.route="success";render();if(matchMedia("(max-width:767px)").matches)window.location.assign(url);else window.open(url,"_blank","noopener");});
    view.querySelectorAll("[data-builder]").forEach(button=>button.addEventListener("click",()=>{const key=button.dataset.builder;if(key==="type"&&state.builder.type!==button.dataset.value)state.builder.details={personalizacion:[]};state.builder[key]=button.dataset.value;if(key==="type")state.builder.use=button.dataset.value;render({scroll:false,entry:false});}));
