(() => {
  ...
  const state = {
    ...
    customerPhone:"",customerEmail:"",customerDeliveryMethod:"shipping",customerDepartment:"Montevideo",customerCity:"",customerAddress:"",customerDeliveryNotes:"",submittedRequestId:"",favorites:new Set(readStorage(STORAGE_FAVORITES,[])),editingUid:"",origin:null,messageWasCompacted:false,builderStep:0,detailStep:0,
  ...
  function checkout(){
    ...
    return `<section class="simple-page checkout-page"><div class="page-head" data-entry><button class="round" data-route="request" aria-label="Volver a mi solicitud"><i class="ph ph-arrow-left" aria-hidden="true"></i></button><h1>Enviar consulta</h1><span></span></div>
      <div class="checkout-grid"><div class="contact-panel" data-entry><span class="section-label">TUS DATOS</span><h2>¿Con quién hablamos?</h2><label>Nombre<input id="customer-name" maxlength="80" value="${escapeText(state.customerName)}" placeholder="Tu nombre"></label><div class="contact-row"><label>WhatsApp<input id="customer-phone" inputmode="tel" maxlength="30" value="${escapeText(state.customerPhone)}" placeholder="099 000 000"></label><label>Email <small>(opcional)</small><input id="customer-email" type="email" maxlength="120" value="${escapeText(state.customerEmail)}" placeholder="nombre@correo.com"></label></div><label>Marca, empresa o evento <small>(opcional)</small><input id="customer-context" maxlength="100" value="${escapeText(state.customerContext)}" placeholder="Ej.: Café Centro"></label><section class="delivery-form"><span class="delivery-title">¿Cómo querés recibirlo?</span><div class="delivery-options" role="group" aria-label="Forma de entrega"><button type="button" data-delivery-method="shipping" class="${state.customerDeliveryMethod==="shipping"?"active":""}"><i class="ph ph-truck"></i><span><b>Envío</b><small>Lo llevamos a tu ubicación</small></span></button><button type="button" data-delivery-method="pickup" class="${state.customerDeliveryMethod==="pickup"?"active":""}"><i class="ph ph-storefront"></i><span><b>Retiro</b><small>Coordinamos cuando esté pronto</small></span></button><button type="button" data-delivery-method="coordinate" class="${state.customerDeliveryMethod==="coordinate"?"active":""}"><i class="ph ph-chat-circle-dots"></i><span><b>A coordinar</b><small>Lo definimos por WhatsApp</small></span></button></div><div class="delivery-address ${state.customerDeliveryMethod==="shipping"?"":"is-hidden"}"><div class="contact-row"><label>Departamento<input id="customer-department" maxlength="80" value="${escapeText(state.customerDepartment)}" placeholder="Ej.: Montevideo"></label><label>Localidad o barrio<input id="customer-city" maxlength="100" value="${escapeText(state.customerCity)}" placeholder="Ej.: Pocitos"></label></div><label>Dirección de entrega<input id="customer-address" maxlength="180" value="${escapeText(state.customerAddress)}" placeholder="Calle, número y apartamento"></label><label>Indicaciones para la entrega <small>(opcional)</small><input id="customer-delivery-notes" maxlength="180" value="${escapeText(state.customerDeliveryNotes)}" placeholder="Horario, portería o referencia"></label></div></section><label>Algo más que debamos saber <small>(opcional)</small><textarea id="customer-message" maxlength="500" rows="3" placeholder="Uso, medidas, fecha o cualquier detalle útil">${escapeText(state.customerMessage)}</textarea></label><p><i class="ph ph-shield-check" aria-hidden="true"></i> La solicitud queda registrada de forma segura para poder responderte y darle seguimiento. No hay pago online: confirmamos precio, material, plazo y entrega o retiro antes de fabricar.</p></div>
  ...
  function siteFooter(){
    return `<footer class="site-footer" id="contacto" data-entry><!-- ATRY Agency temporalmente desactivado. Restaurar href: https://rodrigobrun.github.io/ATRYAGENCY/ --><div class="atry-network agency-disabled" aria-disabled="true"><span><small>ATRY ECOSYSTEM</small><strong>Creamos productos. También construimos marcas.</strong></span><span class="atry-network-destination">ATRY Agency · próximamente <i class="ph ph-lock-simple" aria-hidden="true"></i></span></div><div class="footer-identity"><span class="footer-brand"><img src="recursos/atry-isotipo.png" alt=""><b>ATRY LAB</b></span><p>Diseño y producción 3D personalizada en Montevideo.</p></div><div class="footer-social"><small>ENCONTRANOS EN</small><nav aria-label="Redes y contacto"><a href="https://www.instagram.com/atrylab?stkn=MXhvcWVobW5sdTJlOA==" target="_blank" rel="noopener"><i class="ph ph-instagram-logo"></i> Instagram <span class="outbound-icon" data-morph="outbound"><i class="ph ph-arrow-up-right" aria-hidden="true"></i></span></a><a href="https://www.tiktok.com/@atry.lab" target="_blank" rel="noopener" aria-label="TikTok de ATRY LAB, @atry.lab"><i class="ph ph-tiktok-logo"></i> TikTok <span class="outbound-icon" data-morph="outbound"><i class="ph ph-arrow-up-right" aria-hidden="true"></i></span></a><a href="mailto:atryagency@gmail.com"><i class="ph ph-envelope-simple"></i> Email</a><a href="https://wa.me/${PHONE}" target="_blank" rel="noopener"><i class="ph ph-whatsapp-logo"></i> WhatsApp <span class="outbound-icon" data-morph="outbound"><i class="ph ph-arrow-up-right" aria-hidden="true"></i></span></a></nav></div><small class="footer-copy">© 2026 ATRY LAB · Montevideo, Uruguay</small></footer>`;
  }
  ...
  function buildMessage(compact=false){
    ...
    lines.push("","ENTREGA");
    if(state.customerDeliveryMethod==="shipping"){
      lines.push("- Modalidad: Envío",`- Ubicación: ${[state.customerAddress,state.customerCity,state.customerDepartment].filter(Boolean).join(", ")}`);
      if(state.customerDeliveryNotes)lines.push(`- Indicaciones: ${trimMessage(state.customerDeliveryNotes,180)}`);
    }else if(state.customerDeliveryMethod==="pickup")lines.push("- Modalidad: Retiro coordinado");
    else lines.push("- Modalidad: A coordinar por WhatsApp");
  ...
  async function sendWhatsAppRequest(message,targetWindow=null){
    ...
    if(canShareFiles){
      try{if(targetWindow&&!targetWindow.closed)targetWindow.close();await navigator.share(shareData);return true;}
    ...
    else if(matchMedia("(max-width:767px)").matches)window.location.assign(url);
    else window.location.href=url;
    return true;
  }

  function openWhatsAppConfirmation(onConfirm){
    document.querySelector(".whatsapp-confirm-backdrop")?.remove();
    const modal=document.createElement("div");modal.className="whatsapp-confirm-backdrop";
    modal.innerHTML=`<section class="whatsapp-confirm" role="dialog" aria-modal="true" aria-labelledby="whatsapp-confirm-title"><button class="whatsapp-confirm-close" type="button" aria-label="Cerrar"><i class="ph ph-x"></i></button><span class="whatsapp-confirm-icon"><i class="ph ph-whatsapp-logo"></i></span><small>ÚLTIMO PASO</small><h2 id="whatsapp-confirm-title">¿Enviar esta solicitud?</h2><p>Vamos a registrar la consulta y abrir WhatsApp con el mensaje completo listo para enviar.</p><div class="whatsapp-confirm-note"><i class="ph ph-info"></i><span><b>Importante</b>Cuando se abra WhatsApp, tocá <strong>Enviar</strong>. Así recibimos tu mensaje y podemos responderte a la brevedad.</span></div><div class="whatsapp-confirm-summary"><span><i class="ph ph-stack"></i>${state.items.length} ${state.items.length===1?"idea":"ideas"}</span><span><i class="ph ph-map-pin"></i>${escapeText(state.customerDeliveryMethod==="shipping"?`${state.customerCity}, ${state.customerDepartment}`:state.customerDeliveryMethod==="pickup"?"Retiro coordinado":"Entrega a coordinar")}</span></div><footer><button type="button" class="secondary-button" data-confirm-cancel>Revisar datos</button><button type="button" class="whatsapp-confirm-send" data-confirm-send><i class="ph ph-whatsapp-logo"></i>Abrir WhatsApp</button></footer></section>`;
    const close=()=>{modal.remove();document.body.classList.remove("confirm-open");};
    document.body.append(modal);document.body.classList.add("confirm-open");
    modal.querySelector(".whatsapp-confirm-close").onclick=close;modal.querySelector("[data-confirm-cancel]").onclick=close;modal.onclick=event=>{if(event.target===modal)close();};
    modal.querySelector("[data-confirm-send]").onclick=async event=>{const button=event.currentTarget;button.disabled=true;button.innerHTML='<i class="ph ph-circle-notch"></i> Preparando';try{await onConfirm();close();}catch(error){console.error(error);button.disabled=false;button.innerHTML='<i class="ph ph-whatsapp-logo"></i>Abrir WhatsApp';notify("No pudimos preparar el mensaje. Probá nuevamente.");}};
    modal.querySelector("[data-confirm-send]").focus({preventScroll:true});
  }
  ...
  function bindEvents(){
    ...
    view.querySelectorAll("[data-delivery-method]").forEach(button=>button.addEventListener("click",()=>{state.customerDeliveryMethod=button.dataset.deliveryMethod;view.querySelectorAll("[data-delivery-method]").forEach(item=>item.classList.toggle("active",item===button));view.querySelector(".delivery-address")?.classList.toggle("is-hidden",state.customerDeliveryMethod!=="shipping");}));
    view.querySelector("#send-whatsapp")?.addEventListener("click",()=>{state.customerName=view.querySelector("#customer-name").value.trim();state.customerPhone=view.querySelector("#customer-phone").value.trim();state.customerEmail=view.querySelector("#customer-email").value.trim();state.customerContext=view.querySelector("#customer-context").value.trim();state.customerDepartment=view.querySelector("#customer-department")?.value.trim()||state.customerDepartment;state.customerCity=view.querySelector("#customer-city")?.value.trim()||"";state.customerAddress=view.querySelector("#customer-address")?.value.trim()||"";state.customerDeliveryNotes=view.querySelector("#customer-delivery-notes")?.value.trim()||"";state.customerMessage=view.querySelector("#customer-message").value.trim();if(!state.customerName){notify("Decinos tu nombre para continuar");view.querySelector("#customer-name").focus();return;}if(state.customerPhone.replace(/\D/g,"").length<8){notify("Ingresá un WhatsApp válido para poder responderte");view.querySelector("#customer-phone").focus();return;}if(state.customerDeliveryMethod==="shipping"&&!state.customerCity){notify("Indicá la localidad o el barrio para el envío");view.querySelector("#customer-city")?.focus();return;}if(state.customerDeliveryMethod==="shipping"&&!state.customerAddress){notify("Indicá la dirección donde querés recibirlo");view.querySelector("#customer-address")?.focus();return;}const checkoutButton=view.querySelector("#send-whatsapp");openWhatsAppConfirmation(async()=>{const popup=!matchMedia("(max-width:767px)").matches?window.open("about:blank","_blank"):null;checkoutButton.disabled=true;checkoutButton.innerHTML='<i class="ph ph-circle-notch"></i> Registrando solicitud';try{await registerPublicRequest();notify(`Solicitud ${state.submittedRequestId} registrada`);}catch(error){console.error(error);notify("No pudimos registrarla en el panel; igual podés enviarla por WhatsApp");}let message=buildMessage();state.messageWasCompacted=encodeURIComponent(message).length>7000;if(state.messageWasCompacted)message=buildMessage(true);const sent=await sendWhatsAppRequest(message,popup);checkoutButton.disabled=false;if(sent){state.route="success";navigateToRoute("success",{renderRoute:false});render();}});});
